...
/*
        $sql = "select distinct m.idMenu,m.nombre, m.accion as formulario, ifnull(a.certificadotrabajo,0) certificadotrabajo from menu m inner join menupermisos mp on m.idMenu=mp.idMenu ";
        $sql .= " left join permisosroles pr on mp.idPermiso=pr.idpermiso ";
        $sql .= " left join personarol pro on pro.idrol=" . $this->Session->getIdRole() . " and pro.idpersona=" . $this->Session->getIdPersona();
        $sql .= " left join alumnos a on a.idpersona=" . $this->Session->getIdPersona();
        $sql .= " where m.activo=1 and pr.idrol=" . $this->Session->getIdRole();
        $sql .= " order by m.nombre asc";
  */      
        $sql= " select distinct m.idMenu,m.nombre, m.accion as formulario from menu m inner join menupermisos mp on m.idMenu=mp.idMenu ";
        $sql.=" where mp.idpermiso in ( ";
        $sql.=" select idpermiso from personapermisos where idpersona=". $this->Session->getIdPersona();
        $sql.=" union all select idpermiso from permisosroles where idrol=". $this->Session->getIdRole();
